...
from problems.problem_26 import Problem26
from problems.problem_28 import Problem28
from problems.problem_37 import Problem37
from problems.problem_820 import Problem820

from tests.test_case_problems_methods import TEST_CASE_PROBLEM_21_D, TEST_CASE_PROBLEM_26_LENGTH_RECURRING_CYCLE, \
    TEST_CASE_PROBLEM_820_D, TEST_CASE_PROBLEM_820_S
...
@pytest.mark.parametrize("fraction, expected_result", TEST_CASE_PROBLEM_26_LENGTH_RECURRING_CYCLE)
def test_problem_26_length_recurring_cycle(fraction, expected_result):
    assert Problem26().length_recurring_cycle(numerator=fraction[0], denominator=fraction[1]) == expected_result


@pytest.mark.parametrize("input, expected_result", [([3797, 'left'], True), ([3797, 'right'], True)])
def test_problem_37_is_a_truncable_prime(input, expected_result):
    assert Problem37().is_a_truncable_prime(number=input[0], _from=input[1]) == expected_result
